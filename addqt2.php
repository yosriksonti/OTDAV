...
if($_SERVER['REQUEST_METHOD'] == 'POST' )
{
    ...
    $qt_genre = "Genre ".mysqli_real_escape_string($mysqli, $_POST['genre']);
        ...
        $ID="";
        $ID=$num;
        $result = mysqli_query($mysqli, "INSERT INTO quitance VALUES('$ID','$qt_namefr','$qt_sum','$qt_sumL','$qt_mode','$qt_titre','$qt_genre','$qt_date_c','$qt_date_e','$dpid',0)");
...
pdf.save('quittance-".$ID.".pdf');
window.location.href = 'new_upload1.php?ID=".$dpid."';
</script>";
}
$pdf = new Fpdi();
$pageCount = $pdf->setSourceFile('tmp/depot/depot-'.$dpid.'.pdf');
$pageId = $pdf->importPage(1, PdfReader\PageBoundaries::MEDIA_BOX);
...
$pdf->Output('tmp/depot/depot-'.$dpid.'.pdf','F');
}
